...
void loop() {
  ...
  update_temp();
  mqttClient.publish("ENTC-DHT22-TEMP_KN", TEMP_ARRAY);
  delay(50);

  update_humid();
  mqttClient.publish("ENTC-DHT22-HUMID_KN", HUMID_ARRAY);
  delay(50);

  updateLight();
  mqttClient.publish("ENTC-LDR-LEFT_KN", LEFT_LDR_ARR);
  delay(50);
  mqttClient.publish("ENTC-LDR-RIGHT_KN", RIGHT_LDR_ARR);
  delay(100);
...
void update_humid() {
  TempAndHumidity data = dhtSensor.getTempAndHumidity();
  String(data.humidity, 2).toCharArray(HUMID_ARRAY, 6);
}
...
void connectToBroker() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect("ESP32Client-1234567890")) {
      Serial.println("MQTT Connected");
      mqttClient.subscribe("ADUJST-SERVO_MINIMUM_ANGLE_KN");
      mqttClient.subscribe("ADJUST-SERVO_CONTROL_FACTOR_KN");
    } else {
...
void recieveCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strcmp(topic, "ADUJST-SERVO_MINIMUM_ANGLE_KN") == 0) {
    t_off = String(payloadCharAr).toInt();
  } else if (strcmp(topic, "ADJUST-SERVO_CONTROL_FACTOR_KN") == 0) {
    GAMMA = String(payloadCharAr).toFloat();
  }
}

//////////////////////
// Update light levels and adjust servo angle according to that
//////////////////////

void updateLight() {
  float LEFT_SIDEVAL = analogRead(LDR_LEFT) * 1.00;
  float RIGHT_SIDEVAL = analogRead(LDR_RIGHT) * 1.00;
  float LEFT_VALUES = (LEFT_SIDEVAL - 4063.00) / (32.00 - 4063.00);
  float RIGHT_VALUES = (RIGHT_SIDEVAL - 4063.00) / (32.00 - 4063.00);

  sprintf(LEFT_LDR_ARR, "%.2f", LEFT_VALUES);
  sprintf(RIGHT_LDR_ARR, "%.2f", RIGHT_VALUES);

  Serial.println("left");
  Serial.print(LEFT_LDR_ARR);
  Serial.println("right");
  Serial.print(RIGHT_LDR_ARR);


  updateAngle(LEFT_VALUES, RIGHT_VALUES);
}

void updateAngle(float LEFT_SIDEVAL, float RIGHT_SIDEVAL) {
  float max_I = LEFT_SIDEVAL;
  float D = 1.5;
  if (RIGHT_SIDEVAL > max_I) {
    max_I = RIGHT_SIDEVAL;
    D = 0.5;
  }
  int theta = t_off * D + (180 - t_off) * max_I * GAMMA;
  theta = min(theta, 180);
  servo.write(theta);
}
